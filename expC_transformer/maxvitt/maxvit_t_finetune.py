# maxvit_t_finetune.py
# ==========================================================
# MaxViT-T - FINE-TUNING (2-stage)
# Stage 1: freeze backbone (warmup)
# Stage 2: unfreeze last PERCENT params (lebih robust tanpa tahu struktur internal)
# ==========================================================
...
def unfreeze_last_percent(model, pct=0.2):
    # buka last pct dari parameter (robust)
    params = list(model.named_parameters())
    k = int(len(params) * pct)
    if k < 1: k = 1
    # default: semua tetap frozen dulu
    for _, p in params:
        p.requires_grad = False
    # classifier selalu trainable
    for p in model.get_classifier().parameters():
        p.requires_grad = True
    # unfreeze last k params
    for name, p in params[-k:]:
        p.requires_grad = True

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_dir", required=True, type=str)
    ap.add_argument("--out_dir", default="out_maxvit_t_finetune", type=str)
    ...
    ap.add_argument("--unfreeze_pct", default=0.2, type=float)  # 20% terakhir
    ...
    model = timm.create_model("maxvit_tiny_tf_224", pretrained=True, num_classes=num_classes).to(device)
    ...
    freeze_all(model)
    opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr_warmup, weight_decay=1e-4)
    ...
    for epoch in range(1, args.warmup_epochs+1):
        ...
        print(f"[Warmup] Epoch {epoch:02d} | tr {tr_loss:.4f}/{tr_acc:.4f} | val {va_loss:.4f}/{va_acc:.4f}")
        if va_acc > best_val:
            best_val = va_acc; torch.save(model.state_dict(), best_path); bad = 0
        else:
            bad += 1

    # Stage 2: finetune (unfreeze last pct)
    unfreeze_last_percent(model, pct=args.unfreeze_pct)
    opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr_finetune, weight_decay=1e-4)
    ...
    for epoch in range(1, args.finetune_epochs+1):
        ...
        hist["train_loss"].append(tr_loss); hist["train_acc"].append(tr_acc)
        hist["val_loss"].append(va_loss);   hist["val_acc"].append(va_acc)
        print(f"[Finetune] Epoch {epoch:02d} | tr {tr_loss:.4f}/{tr_acc:.4f} | val {va_loss:.4f}/{va_acc:.4f}")
    ...
    plt.title("Confusion Matrix - MaxViT-T Fine-tuning")
    plt.savefig(figs/"confusion_matrix.png", dpi=200, bbox_inches="tight")
    plt.close()

    # top errors
    from PIL import Image
    # rebuild quick for error grid
    ...
    print(f"[MaxViT FINETUNE] Test Acc={te_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
