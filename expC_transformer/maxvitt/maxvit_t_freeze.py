# maxvit_t_freeze.py
# ==========================================================
# MaxViT-T (timm) - FREEZE backbone (train head only)
# Output:
...
def plot_top_errors_from_test(model, test_ds, test_dl, device, class_names, out_png: Path, topk=12):
    # ambil confidence tertinggi pada prediksi SALAH
    model.eval()
    wrong_imgs = []
    wrong_titles = []
    with torch.no_grad():
        ...
        for x, y in test_dl:
            ...
            for i in range(len(y_np)):
                if pred[i] != y_np[i]:
                    wrong_imgs.append((idx_global+i, conf[i], pred[i], y_np[i]))
            idx_global += len(y_np)

    if len(wrong_imgs) == 0:
        return

    wrong_imgs.sort(key=lambda t: t[1], reverse=True)
    pick = wrong_imgs[:topk]

    cols = 4
    rows = int(np.ceil(len(pick)/cols))
    plt.figure(figsize=(4*cols, 4*rows))

    # untuk tampilkan gambar, pakai versi non-normalized (reload PIL)
...
def freeze_backbone(model):
    # timm classifier head biasanya: model.get_classifier() / reset_classifier
    for p in model.parameters():
        p.requires_grad = False
    # nyalakan params head classifier
...
def main():
    ...
    ap.add_argument("--out_dir", default="out_maxvit_t_freeze", type=str)
    ...
    plt.title("Confusion Matrix - MaxViT-T Freeze")
    plt.savefig(figs/"confusion_matrix.png", dpi=200, bbox_inches="tight")
    plt.close()

    plot_top_errors_from_test(model, test_ds, test_dl, device, class_names, figs/"top_errors.png", topk=12)
    ...
    print(f"[MaxViT FREEZE] Test Acc={te_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
