# vit_b16_lora.py
# ==========================================================
# ViT-B/16 - LoRA (replace nn.Linear) + train LoRA + head
# Output lengkap sama
# ==========================================================
...
def replace_linear_with_lora(model: nn.Module, r=8, alpha=16, dropout=0.0):
    for name, module in list(model.named_children()):
        if isinstance(module, nn.Linear):
            # jangan LoRA-kan classifier head (biarkan normal trainable)
            if "head" in name or "classifier" in name:
                continue
            setattr(model, name, LoRALinear(module, r=r, alpha=alpha, dropout=dropout))
        else:
            replace_linear_with_lora(module, r=r, alpha=alpha, dropout=dropout)
...
def main():
    ...
    ap.add_argument("--out_dir", default="out_vit_b16_lora", type=str)
    ...
    replace_linear_with_lora(model, r=args.r, alpha=args.alpha, dropout=args.lora_dropout)
    ...
    plt.title("Confusion Matrix - ViT-B/16 LoRA")
    ...
    print(f"[ViT-B/16 LoRA] Test Acc={te_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
