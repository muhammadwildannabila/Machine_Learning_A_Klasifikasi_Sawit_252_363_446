# gabor_extratrees.py
# ============================================
# Feature Extraction (GABOR) + ExtraTrees
# ============================================
...
def gabor_bank_features(gray_u8,
                        thetas=(0, np.pi/4, np.pi/2, 3*np.pi/4),
                        freqs=(0.1, 0.2, 0.3),
                        ksize=31):
    g = gray_u8.astype(np.float32) / 255.0
    feats = [float(g.mean()), float(g.std())]
    for theta in thetas:
        for freq in freqs:
            lambd = 1.0 / freq
            kernel = cv2.getGaborKernel((ksize, ksize), 4.0, float(theta), float(lambd), 0.5, 0, ktype=cv2.CV_32F)
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="out_gabor_et")
    ...
    model = ExtraTreesClassifier(random_state=args.seed, n_jobs=-1)
    ...
    plot_cv_results(grid, out_dir/"figs"/"cv_top10.png", "GABOR + ExtraTrees CV Top-10 (macro-F1)")
    ...
    plt.title("Confusion Matrix - GABOR + ExtraTrees")
    ...
    summary = {
        ...
        "gabor": {"thetas": 4, "freqs": 3, "ksize": 31, "stats_per_filter": 2}
    }
