# gabor_xgboost.py
# ============================================
# Feature Extraction (GABOR) + XGBoost
# ============================================
...
def gabor_bank_features(gray_u8,
    ...
    feats = []
    feats.append(float(g.mean()))
    feats.append(float(g.std()))

    for theta in thetas:
        for freq in freqs:
            # hubungan freq -> lambda (wavelength)
            lambd = 1.0 / freq
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="out_gabor_xgb")
    ...
    plot_cv_results(grid, out_dir/"figs"/"cv_top10.png", "GABOR + XGBoost CV Top-10 (macro-F1)")
    ...
    plt.title("Confusion Matrix - GABOR + XGBoost")
