# texture_xgboost.py
# ============================================
# Feature Extraction (TEXTURE: GLCM + LBP) + XGBoost
# ============================================
...
def texture_features(gray_u8, glcm_levels=32, distances=(1,2), angles=(0, np.pi/4, np.pi/2, 3*np.pi/4),
    ...
    g = (g / 256.0 * glcm_levels).astype(np.uint8)
    ...
    feats = []
    for pr in props:
        vals = graycoprops(glcm, pr)  # shape (len(dist), len(angle))
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="out_texture_xgb")
    ...
    plot_cv_results(grid, out_dir/"figs"/"cv_top10.png", "TEXTURE + XGBoost CV Top-10 (macro-F1)")
    ...
    plt.title("Confusion Matrix - TEXTURE + XGBoost")
