# efficientnetb0_freeze.py
# ==========================================================
# EfficientNetB0 (ImageNet) - FREEZE backbone
...
def build_model(num_classes, img_size=224, dropout=0.3):
    ...
    model = keras.Model(inp, out, name="efficientnetb0_freeze")
    return model

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_dir", required=True, type=str)
    ap.add_argument("--out_dir", default="out_efficientnetb0_freeze", type=str)
    ...
    plt.title("Confusion Matrix - EfficientNetB0 Freeze")
    plt.savefig(figs/"confusion_matrix.png", dpi=200, bbox_inches="tight")
    plt.close()

    # images for error grid
    imgs = []
    for bx, _ in test_ds:
    ...
    print(f"[FREEZE] Test Acc={test_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
