# resnet50_lora.py
# ==========================================================
# ResNet50 (ImageNet) - LoRA Adapter Head (PEFT)
# Backbone: freeze
# Trainable: LoRA A & B pada head logits
# ==========================================================
...
class LoRAHead(layers.Layer):
    def __init__(self, num_classes, r=8, alpha=16, **kwargs):
        ...
        self.A = layers.Dense(r, use_bias=False, name="lora_A")
        self.B = layers.Dense(num_classes, use_bias=False, name="lora_B")
...
def build_model(num_classes, img_size=224, dropout=0.3, r=8, alpha=16):
    ...
    return keras.Model(inp, out, name="resnet50_lora_head")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_dir", required=True, type=str)
    ap.add_argument("--out_dir", default="out_resnet50_lora", type=str)
    ...
    plt.title("Confusion Matrix - ResNet50 LoRA (Head)")
    ...
    print(f"[ResNet50 LoRA-HEAD] Test Acc={test_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
