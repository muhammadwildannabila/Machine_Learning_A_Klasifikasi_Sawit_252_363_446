# resnet50_freeze.py
# ==========================================================
# ResNet50 (ImageNet) - FREEZE backbone
# Output: history_acc_loss.png, confusion_matrix.png, classification_report.txt,
#         top_errors.png, best_model.keras, summary.json
# ==========================================================
...
def build_model(num_classes, img_size=224, dropout=0.3):
    ...
    x = layers.Lambda(tf.keras.applications.resnet.preprocess_input)(inp)
    ...
    return keras.Model(inp, out, name="resnet50_freeze")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_dir", required=True, type=str)
    ap.add_argument("--out_dir", default="out_resnet50_freeze", type=str)
    ...
    model = build_model(len(class_names), img_size=args.img_size)
    model.compile(
    ...
    plt.title("Confusion Matrix - ResNet50 Freeze")
    ...
    print(f"[ResNet50 FREEZE] Test Acc={test_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
