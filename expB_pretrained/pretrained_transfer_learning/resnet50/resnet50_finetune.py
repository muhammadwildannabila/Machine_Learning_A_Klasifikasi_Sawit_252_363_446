...
# ==========================================================
# ResNet50 (ImageNet) - FINE-TUNING (2-stage)
# Stage 1: freeze (warmup)
# Stage 2: unfreeze last N layers (BatchNorm tetap freeze)
# ==========================================================
...
def build_model(num_classes, img_size=224, dropout=0.3):
    ...
    model = keras.Model(inp, out, name="resnet50_finetune")
    return model, base

def freeze_all(base):
    base.trainable = False

def unfreeze_last_layers(base, n_last=40):
    base.trainable = True
...
def main():
    ...
    ap.add_argument("--out_dir", default="out_resnet50_finetune", type=str)
    ...
    print(f"[ResNet50 FINETUNE] Test Acc={test_acc:.4f} | Macro-F1={mf1:.4f} | Saved -> {out_dir}")
